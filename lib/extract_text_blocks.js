...
var isTagName = function(property) {
  return property.key === "tagName";
};
 ...
 *
 * @return {String} i18n.tagName
 *         The type of tag the stringValue should be wrapped in. Defaults to
 *         "div", can be overridden via attribute on the <Text/> or via global
 *         configure call.
 */
var extract = function(textBlock, dontCompile) {
  var extractParam = function(params, testFn) {
    var index = -1;
    params.some(function(param, i) {
      if (testFn(param)) {
        index = i;
        return true;
      }
    ...
    if (index >= 0) {
      return params.splice(index, 1)[0].value;
    }
  };
  ...
  var i18nKey = extractParam(tagParams, isKey);
  var outputTagName = extractParam(tagParams, isTagName) || config.tagName;
  var i18nParams = tagParams.reduce(function(set, entry) {
